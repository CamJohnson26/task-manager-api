# worker_api/routes/get_completed_tasks.py

from flask import Blueprint, jsonify
from authlib.integrations.flask_oauth2 import current_token

from auth0.auth_protector import require_auth
from task_manager_db.db_actions import get_user_by_sub_db, get_completed_tasks_db

get_completed_tasks_bp = Blueprint('completed_tasks', __name__)


@get_completed_tasks_bp.route('/tasks/completed')
@require_auth()
def get_completed_tasks():
    ...
    completed_tasks = get_completed_tasks_db(user_id)
    ...
    for task in completed_tasks:
        ...
        if task[4] == "recurring" and task[11]:  # task[4] is type, task[10] is last_completed
